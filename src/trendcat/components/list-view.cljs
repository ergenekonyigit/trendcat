...
(def since-cursor (r/cursor app-state [:since]))

(defn list-item [repo]
  [:div {:style {:margin-bottom "20px"
                 :border-bottom "1px solid #e8e8e8"}}
   [:div {:style {:margin-bottom "15px"}} ;; header
   ...
   [:div {:style {:margin-bottom "15px"
    ...
    (when (:language repo)
      [:span {:style {:display "inline-flex"
                      ...
                      :margin-right "30px"
                      :color "#212529"}}
       [:span {:style {:background-color (:languageColor repo)
                       :border-radius "50%"
                       :display "inline-block"
                       :height "12px"
                       :position "relative"
                       ...
                       :margin-right "4px"}}]
       [:span (:language repo)]])
    [:a {:href (str (:url repo) "/stargazers")
         :target "_blank"
         :style {:display "inline-flex"
                 ...
                 :font-weight "500"
                 :color "#6c757d"}}
     [:svg {:view-box "0 0 14 16" :version "1.1" :width "14" :height "16" :role "img"}
      [:path {:fill "#6c757d" :fill-rule "evenodd" :d "M14 6l-4.9-.64L7 1 4.9 5.36 0 6l3.6 3.26L2.67 14 7 11.67 11.33 14l-.93-4.74L14 6z"}]]
     \u00A0 (:stars repo)]
    [:a {:href (str (:url repo) "/network/members")
         :target "_blank"
         :style {:display "inline-flex"
                 :flex-direction "row"
                 :align-items "center"
                 ...
                 :font-weight "500"
                 :color "#6c757d"}}
     [:svg {:view-box "0 0 10 16" :version "1.1" :width "10" :height "16" :role "img"}
      [:path {:fill "#6c757d" :fill-rule "evenodd" :d "M8 1a1.993 1.993 0 0 0-1 3.72V6L5 8 3 6V4.72A1.993 1.993 0 0 0 2 1a1.993 1.993 0 0 0-1 3.72V6.5l3 3v1.78A1.993 1.993 0 0 0 5 15a1.993 1.993 0 0 0 1-3.72V9.5l3-3V4.72A1.993 1.993 0 0 0 8 1zM2 4.2C1.34 4.2.8 3.65.8 3c0-.65.55-1.2 1.2-1.2.65 0 1.2.55 1.2 1.2 0 .65-.55 1.2-1.2 1.2zm3 10c-.66 0-1.2-.55-1.2-1.2 0-.65.55-1.2 1.2-1.2.65 0 1.2.55 1.2 1.2 0 .65-.55 1.2-1.2 1.2zm3-10c-.66 0-1.2-.55-1.2-1.2 0-.65.55-1.2 1.2-1.2.65 0 1.2.55 1.2 1.2 0 .65-.55 1.2-1.2 1.2z"}]]
     \u00A0 (:forks repo)]
    [:span {:style {:display "inline-flex"
                    :flex-direction "row"
                    :align-items "center"
                    ...
                    :font-weight "500"
                    :color "#6c757d"}}
     [:svg {:view-box "0 0 1024 1024", :version "1.1", :width "18", :height "18"}
   ...
   {:display-name "list-view"
    ...
    (fn []
      [:div {:style {:padding "35px"
       ...
       (if-let [trends (:trending @app-state)]
         (for [repo trends]
           ^{:key (:url repo)}
           [list-item repo])
         [:div {:style {:color "#000"
